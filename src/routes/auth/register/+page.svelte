	...
	<div class="flex flex-col w-full">
		<form on:submit|preventDefault={onRegister}>
			<div class:hidden={otherField}>
				...
				<div class="flex flex-col items-center">
					<Button
						on:click={checkOtherField}
						class={isFilledNameAndEmail
							? 'w-full max-w-sm mt-5'
							: ' btn-disabled w-full max-w-sm mt-5'}
					>
						Continuer
					</Button>
					<div class="flex justify-center w-full max-w-sm items-center mt-6">
						<span class="text-sm xl:text-md mr-3"> Déjà un compte ? </span>
						<a href="/auth/login" class="text-sm text-gray-500 hover:text-primary">
							Se connecter!
						</a>
					</div>
				</div>
			...
			<div class:hidden={!otherField}>
				...
				<div class="flex flex-col">
					<div class="flex justify-center">
						<Button on:click={() => (otherField = !otherField)} class=" w-1/3 btn-accent mr-10 mt-5"
							>retour</Button
						>
						<Button type="submit" class={isFilled ? 'w-1/3 mt-5' : 'btn-disabled w-1/3 mt-5'}
							>S'inscrire</Button
						>
					</div>
