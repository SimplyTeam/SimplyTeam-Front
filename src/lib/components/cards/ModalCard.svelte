...
<dialog class="rounded" bind:this={dialog} on:close={() => (showModal = false)} on:click|self={() => dialog.close()}>
	<div class="flex justify-center h-full !p-0 flex-1">
		<div class="w-6/12">
			<div class="flex flex-col h-full px-10 pt-10 justify-between items-center">
				...
				<div class="flex flex-col pb-10 w-full">
					<div class="flex flex-col mt-5 items-center">
						<Input bind:value={payload.name} placeholder="" labelInput="Nom de l'espace de travail" name="email" type="email" />
					</div>
					<div class="flex flex-col mt-5 items-center">
						<Input bind:value={payload.description} placeholder="" labelInput="Description" name="text" type="text" />
					</div>
					<div class="flex mt-5 flex-col items-center w-full">
						<div class="label w-full font-bold">
						...
						<Svelecte
							...
							multiple
							bind:value={payload.invitation}
						/>
					</div>
				</div>
				<div class="flex flex-col items-center w-full">
					<Button on:click={() => createWorkspace()} class="w-full max-w-sm mt-5 mb-10">C'est parti !</Button>
				</div>
			</div>
		</div>
		<div class="flex-1 w-7/12 bg-primary text-start hidden lg:flex">
